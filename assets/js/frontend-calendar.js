jQuery(document).ready(function($) {
    ...
    function podlaczNasluchy() {
        ...
        $('#srl-powrot-krok-4').on('click', function() {
            pokazKrok(4);
        });
        $('#srl-dalej-krok-5').on('click', function() {
            pokazKrok(5);
        });
    ...
    function zaladujDaneKlienta() {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                ukryjKomunikat();
                console.error('❌ [ERROR] Błąd AJAX ładowania danych:', {
                    xhr,
                    status,
                    error
                });
            ...
            if (lot.status === 'zarezerwowany') {
                ...
                if (moznaModyfikowac) {
                    ...
                    if (!lot.ma_filmowanie || lot.ma_filmowanie == '0') {
                        dostepneOpcje.push({
                            nazwa: 'Filmowanie',
                            id: srlFrontend.productIds.filmowanie
                        });
                    }
                    if (!lot.ma_akrobacje || lot.ma_akrobacje == '0') {
                        dostepneOpcje.push({
                            nazwa: 'Akrobacje',
                            id: srlFrontend.productIds.akrobacje
                        });
                    }
                ...
                if (lot.data_waznosci) {
                    ...
                    if (dniDoWaznosci <= 3390) {
                        dostepneOpcje.push({
                            nazwa: 'Przedłużenie',
                            id: srlFrontend.productIds.przedluzenie
                        });
                    }
                }
                if (dostepneOpcje.length > 0) {
