...
jQuery(document).ready(function($) {
    ...
    function wypelnijDaneKlienta() {
        // Wypełnij listę rezerwacji
        wypelnijListeRezerwacji(daneKlienta.rezerwacje);
        
        // Wypełnij dostępne loty
        ...
        // Jeśli dane są kompletne, podświetl przycisk
    ...
    function wypelnijListeRezerwacji(rezerwacje) {
        ...
        rezerwacje.forEach(function(rezerwacja) {
            ...
            var moznaAnulowac = czasDoLotu > 48 * 60 * 60 * 1000; // 48h w ms
    ...
    function wypelnijDostepneLoty(loty) {
        ...
        if (loty.length === 1) {
            ...
            html += '#' + lot.order_id + ' – ' + escapeHtml(lot.nazwa_produktu);
            html += '<input type="hidden" id="srl-wybrany-lot" value="' + lot.id + '">';
            html += '</div>';
        } else {
            html += '<select id="srl-wybrany-lot" required>';
            html += '<option value="">Wybierz lot...</option>';
        ...
        // Pokaż informację o ilości dostępnych lotów
    ...
    // Override pokazKrok dla kroku 4
        ...
        $.ajax({
            ...
            success: function(response) {
                if (response.success) {
                    ...
                    // Odśwież dane klienta po chwili
                    setTimeout(function() {
                        zaladujDaneKlienta();
                        $('#srl-voucher-form').hide();
                        $('#srl-voucher-show').show();
