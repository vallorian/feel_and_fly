...
jQuery(document).ready(function($) {
    ...
    function srl_formatujDanePasazera(slot, dlugiFormat) {
        ...
        if ((slot.status === 'Zarezerwowany' || slot.status === 'Zrealizowany') && slot.klient_nazwa) {
            ...
            if (pelneInfo || daneOdwolane) {
                ...
                var statusTekst = slot.status === 'Zarezerwowany' ? '(ZAREZERWOWANY)' : slot.status === 'Zrealizowany' ? '(ZREALIZOWANY)' : '';
                if (statusTekst) {
    ...
    function generujTabelePilotow() {
        ...
        for (var pid = 1; pid <= liczbaPilotow; pid++) {
            ...
            var tabela = $('<table class="widefat fixed" data-pilot-id="' + pid + '">' + '<thead>' + '<tr>' + '<th style="width:30px;"><input type="checkbox" class="srl-zaznacz-wszystkie-naglowek" data-pilot="' + pid + '"></th>' + '<th style="width:30px;">Nr</th>' + '<th>Czas lotu</th>' + '<th>Status slotu</th>' + '<th>ID lotu</th>' + '<th>Dane pasażera</th>' + '<th>Akcje</th>' + '</tr>' + '</thead>' + '<tbody></tbody>' + '</table>');
    ...
    function dodajWierszDoTabeli(pilotId, numer, obiektGodziny, referencjaTabela) {
        ...
        czasTd.html(czasTxt + ' <button class="button button-small srl-edytuj-button" style="margin-left:10px; font-size:11px;">Edytuj godziny</button>' + ' <button class="button button-secondary button-small srl-usun-button" style="margin-left:5px; font-size:11px;">Usuń</button>');
        tr.append(czasTd);
        var statusTd = $('<td></td>');
        var statusClass = '';
        var statusIcon = '';
    ...
    function zaladujNasluchiwace() {
        ...
        $(document).off('click', '.srl-edytuj-button').on('click', '.srl-edytuj-button', function() {
            ...
            wiersz.find('.srl-czas-col').html('<div style="display:flex; align-items:center; gap:5px; flex-wrap:wrap;">' + '<input type="time" class="srl-edit-start" value="' + currStart + '" style="width:90px;">' + '<span>-</span>' + '<input type="time" class="srl-edit-stop" value="' + currStop + '" style="width:90px;">' + '<button class="button button-small button-primary srl-zapisz-godziny" data-termin-id="' + terminId + '" style="margin-left:5px;">Zapisz</button>' + '<button class="button button-small srl-anuluj-edycje-godzin" style="margin-left:2px;">Anuluj</button>' + '</div>');
        });
    ...
    function getKolorStatusu(status) {
        switch (status) {
            ...
            case 'Prywatny':
                return {
                    bg: '#e2e3e5', border: '#6c757d', text: '#495057'
                };
            case 'Zarezerwowany':
                return {
            ...
            default:
                return {
                    bg: '#f8f9fa', border: '#6c757d', text: '#495057'
                };
        }
    }
    ...
    function pokazUjednoliconyModalDanych(dane, tytul, moznaEdytowac, terminId) {
        ...
        if (moznaEdytowac) {
            ...
            var sprawnoscOpcje = [{
                value: 'zdolnosc_do_marszu',
                label: 'Zdolność do marszu'
            }, {
                value: 'zdolnosc_do_biegu',
                label: 'Zdolność do biegu'
            }, {
                value: 'sprinter',
                label: 'Sprinter!'
            }];
