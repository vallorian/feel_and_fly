<?php add_filter('woocommerce_add_cart_item_data', 'srl_add_flight_option_cart_item_data', 10, 3);
function srl_add_flight_option_cart_item_data($cart_item_data, $product_id, $variation_id) {
    ...
    if (in_array($product_id, $opcje_produkty) && isset($_GET['srl_lot_id'])) {
        ...
        if (is_user_logged_in()) {
            ...
            $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela WHERE id = %d AND user_id = %d", $lot_id, get_current_user_id()), ARRAY_A);
            if ($lot) {
                $cart_item_data['srl_lot_id'] = $lot_id;
                $cart_item_data['srl_lot_info'] = array('nazwa_produktu' => $lot['nazwa_produktu'], 'data_zakupu' => $lot['data_zakupu']);
            }
        }
    }
...
function srl_display_flight_option_cart_item_data($item_data, $cart_item) {
    if (isset($cart_item['srl_lot_id']) && isset($cart_item['srl_lot_info'])) {
        $item_data[] = array('key' => 'Lot do modyfikacji', 'value' => '#' . $cart_item['srl_lot_id'] . ' - ' . $cart_item['srl_lot_info']['nazwa_produktu'], 'display' => '');
    }
    return $item_data;
}
    ...
    foreach ($order->get_items() as $item_id => $item) {
        ...
        if ($lot_id && in_array($product_id, $opcje_produkty)) {
            if ($product_id == $opcje_produkty['przedluzenie']) {
                ...
                if ($lot) {
                    srl_ustaw_opcje_lotu($lot_id, 1, $lot['ma_akrobacje'], "Dokupiono filmowanie (zamówienie #$order_id)");
                }
                ...
                $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela WHERE id = %d", $lot_id), ARRAY_A);
                if ($lot) {
                    srl_ustaw_opcje_lotu($lot_id, $lot['ma_filmowanie'], 1, "Dokupiono akrobacje (zamówienie #$order_id)");
                }
            }
        }
    }
...
function srl_validate_flight_option_add_to_cart($passed, $product_id, $quantity) {
    ...
    if (in_array($product_id, $opcje_produkty)) {
        ...
        $ma_loty = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $tabela WHERE user_id = %d AND status IN ('wolny', 'zarezerwowany')", $user_id));
        if (!$ma_loty) {
