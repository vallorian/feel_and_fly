<?php function srl_generuj_kod_vouchera() {
    ...
    do {
        ...
        $exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $tabela WHERE kod_vouchera = %s", $kod));
    } while ($exists > 0);
    return $kod;
}
    ...
    foreach ($order->get_items() as $item_id => $item) {
        ...
        if ($product && srl_czy_produkt_vouchera($product)) {
            ...
            for ($i = 0;$i < $quantity;$i++) {
                ...
                $result = $wpdb->insert($tabela, array('order_item_id' => $item_id, 'order_id' => $order_id, 'buyer_user_id' => $user_id, 'buyer_imie' => $imie, 'buyer_nazwisko' => $nazwisko, 'nazwa_produktu' => $nazwa_produktu, 'kod_vouchera' => $kod_vouchera, 'status' => 'do_wykorzystania', 'data_zakupu' => $data_zakupu, 'data_waznosci' => $data_waznosci), array('%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s'));
                if ($result !== false) {
                    $dodane_vouchery++;
                    $email_sent = srl_wyslij_email_voucher($email_odbiorcy, $kod_vouchera, $nazwa_produktu, $data_waznosci, $buyer_name);
                    if (!$email_sent) {
...
function srl_usun_vouchery_zamowienia($order_id) {
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_vouchery_upominkowe';
    $usuniete = $wpdb->delete($tabela, array('order_id' => $order_id, 'status' => 'do_wykorzystania'), array('%d', '%s'));
    return $usuniete;
}
function srl_wykorzystaj_voucher($kod_vouchera, $user_id) {
    ...
    $voucher = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_vouchery 
         WHERE kod_vouchera = %s 
         AND status = 'do_wykorzystania' 
         AND data_waznosci >= CURDATE()", $kod_vouchera), ARRAY_A);
    if (!$voucher) {
    ...
    try {
        ...
        $result_lot = $wpdb->insert($tabela_loty, array('order_item_id' => $voucher['order_item_id'], 'order_id' => $voucher['order_id'], 'user_id' => $user_id, 'imie' => $user->first_name ? : 'Voucher', 'nazwisko' => $user->last_name ? : 'User', 'nazwa_produktu' => $voucher['nazwa_produktu'], 'status' => 'wolny', 'data_zakupu' => $data_wykorzystania, 'data_waznosci' => $data_waznosci_lotu, 'ma_filmowanie' => $opcje_produktu['ma_filmowanie'], 'ma_akrobacje' => $opcje_produktu['ma_akrobacje']), array('%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d'));
        if ($result_lot === false) {
        ...
        if ($opcje_produktu['ma_filmowanie'] || $opcje_produktu['ma_akrobacje']) {
            ...
            if (function_exists('srl_ustaw_opcje_lotu')) {
                srl_ustaw_opcje_lotu($lot_id, $opcje_produktu['ma_filmowanie'], $opcje_produktu['ma_akrobacje'], 'Lot z vouchera z opcjami: ' . implode(', ', $opcje_tekst));
            }
        }
        $result_voucher = $wpdb->update($tabela_vouchery, array('status' => 'wykorzystany', 'data_wykorzystania' => $data_wykorzystania, 'wykorzystany_przez_user_id' => $user_id, 'lot_id' => $lot_id), array('id' => $voucher['id']), array('%s', '%s', '%d', '%d'), array('%d'));
        if ($result_voucher === false) {
            throw new Exception('Błąd aktualizacji vouchera.');
        }
        $wpdb->query('COMMIT');
        return array('success' => true, 'message' => 'Voucher został wykorzystany! Lot został dodany do Twojego konta.', 'lot_id' => $lot_id);
    }
    catch(Exception $e) {
        $wpdb->query('ROLLBACK');
