...
function srl_pobierz_loty_uzytkownika($user_id, $status = null) {
    ...
    return $wpdb->get_results($wpdb->prepare("SELECT * FROM $tabela_loty $where_clause ORDER BY data_zakupu DESC", ...$params), ARRAY_A);
}
function srl_pobierz_slot_szczegoly($termin_id) {
    global $wpdb;
    $tabela_terminy = $wpdb->prefix . 'srl_terminy';
    return $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_terminy WHERE id = %d", $termin_id), ARRAY_A);
}
function srl_sprawdz_czy_slot_wolny($termin_id) {
    global $wpdb;
    $tabela_terminy = $wpdb->prefix . 'srl_terminy';
    $count = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $tabela_terminy WHERE id = %d AND status = 'Wolny'", $termin_id));
    return intval($count) > 0;
}
function srl_pobierz_dostepne_sloty_dnia($data) {
    global $wpdb;
    $tabela_terminy = $wpdb->prefix . 'srl_terminy';
    return $wpdb->get_results($wpdb->prepare("SELECT id, pilot_id, godzina_start, godzina_koniec 
         FROM $tabela_terminy 
         WHERE data = %s 
         AND status = 'Wolny'
...
function srl_zwroc_godziny_wg_pilota($data) {
    ...
    $wynik = $wpdb->get_results($wpdb->prepare("SELECT t.id, t.pilot_id, t.godzina_start, t.godzina_koniec, t.status, t.klient_id, t.notatka,
          ...
          WHERE t.data = %s
          ORDER BY t.pilot_id ASC, t.godzina_start ASC", sanitize_text_field($data)), ARRAY_A);
    $noweWgPilota = array();
    foreach ($wynik as $w) {
        ...
        if (($w['status'] === 'Zarezerwowany' || $w['status'] === 'Zrealizowany') && intval($w['klient_id']) > 0) {
            ...
            if ($user) {
                ...
                if (empty($dane_pasazera_cache['imie'])) {
                    $dane_pasazera_cache = array('imie' => $imie, 'nazwisko' => $nazwisko, 'rok_urodzenia' => get_user_meta(intval($w['klient_id']), 'srl_rok_urodzenia', true), 'telefon' => get_user_meta(intval($w['klient_id']), 'srl_telefon', true), 'kategoria_wagowa' => get_user_meta(intval($w['klient_id']), 'srl_kategoria_wagowa', true), 'sprawnosc_fizyczna' => get_user_meta(intval($w['klient_id']), 'srl_sprawnosc_fizyczna', true), 'uwagi' => get_user_meta(intval($w['klient_id']), 'srl_uwagi', true));
                }
            }
        ...
        $noweWgPilota[$pid][] = array('id' => intval($w['id']), 'start' => substr($w['godzina_start'], 0, 5), 'koniec' => substr($w['godzina_koniec'], 0, 5), 'status' => $w['status'], 'klient_id' => intval($w['klient_id']), 'klient_nazwa' => $klient_nazwa, 'link_zamowienia' => $link_zamowienia, 'lot_id' => $w['lot_id'] ? intval($w['lot_id']) : null, 'notatka' => $w['notatka'], 'dane_pasazera_cache' => $dane_pasazera_cache);
    }
    wp_send_json_success(array('godziny_wg_pilota' => $noweWgPilota));
}
...
function srl_pobierz_szczegoly_terminu($termin_id) {
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_terminy';
    $termin = $wpdb->get_row($wpdb->prepare("SELECT data, godzina_start, godzina_koniec FROM $tabela WHERE id = %d", $termin_id), ARRAY_A);
    if ($termin) {
        return sprintf('%s %s-%s', srl_formatuj_date($termin['data']), substr($termin['godzina_start'], 0, 5), substr($termin['godzina_koniec'], 0, 5));
    }
...
function srl_estymuj_date_realizacji($lot) {
    if ($lot['termin_id']) {
        global $wpdb;
        $tabela = $wpdb->prefix . 'srl_terminy';
        $data_lotu = $wpdb->get_var($wpdb->prepare("SELECT data FROM $tabela WHERE id = %d", $lot['termin_id']));
...
function srl_pobierz_historie_opcji($user_id, $lot_id) {
    ...
    $orders = $wpdb->get_results($wpdb->prepare("SELECT ID, post_date 
         FROM {$wpdb->posts} 
         WHERE post_author = %d 
         AND post_type = 'shop_order' 
         AND post_status IN ('wc-completed', 'wc-processing')
         ORDER BY post_date ASC", $user_id), ARRAY_A);
