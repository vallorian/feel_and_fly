        ...
        if ($product && srl_czy_produkt_lotu($product)) {
            ...
            for ($i = 0;$i < $quantity;$i++) {
                ...
                if ($result !== false) {
                    ...
                    $historia_poczatkowa[] = array('data' => $data_zakupu, 'typ' => 'przypisanie_id', 'executor' => 'System', 'szczegoly' => array('lot_id' => $lot_id, 'nazwa_produktu' => $nazwa_produktu, 'order_id' => $order_id, 'order_item_id' => $item_id, 'user_id' => $user_id, 'data_waznosci' => $data_waznosci, 'quantity_info' => $quantity > 1 ? sprintf('%d/%d', $i + 1, $quantity) : '1/1'));
                    if ($opcje['ma_filmowanie']) {
                        $historia_poczatkowa[] = array('data' => $data_zakupu, 'typ' => 'opcja_przy_zakupie', 'executor' => 'System', 'szczegoly' => array('opcja' => 'filmowanie', 'dodano_przy_zakupie' => true, 'order_id' => $order_id));
                    }
                    if ($opcje['ma_akrobacje']) {
                        $historia_poczatkowa[] = array('data' => $data_zakupu, 'typ' => 'opcja_przy_zakupie', 'executor' => 'System', 'szczegoly' => array('opcja' => 'akrobacje', 'dodano_przy_zakupie' => true, 'order_id' => $order_id));
                    }
...
function srl_dokup_filmowanie($lot_id, $order_id, $item_id, $quantity = 1) {
    ...
    if ($result !== false) {
        $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'dokupienie_filmowanie', 'executor' => 'Klient', 'szczegoly' => array('opcja' => 'filmowanie', 'order_id' => $order_id, 'item_id' => $item_id, 'quantity' => $quantity, 'stary_stan' => 0, 'nowy_stan' => 1));
...
function srl_dokup_akrobacje($lot_id, $order_id, $item_id, $quantity = 1) {
    ...
    if ($result !== false) {
        $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'dokupienie_akrobacje', 'executor' => 'Klient', 'szczegoly' => array('opcja' => 'akrobacje', 'order_id' => $order_id, 'item_id' => $item_id, 'quantity' => $quantity, 'stary_stan' => 0, 'nowy_stan' => 1));
        srl_dopisz_do_historii_lotu_v2($lot_id, $wpis_historii);
    }
    return $result !== false;
}
function srl_dokup_przedluzenie($lot_id, $order_id, $item_id = 0, $quantity = 1) {
    ...
    if ($result !== false) {
        $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'przedluzenie_waznosci', 'executor' => 'Klient', 'szczegoly' => array('stara_data_waznosci' => $stara_data, 'nowa_data_waznosci' => $nowa_data, 'przedluzenie_lat' => $quantity, 'order_id' => $order_id, 'item_id' => $item_id, 'quantity_w_zamowieniu' => $quantity));
        srl_dopisz_do_historii_lotu_v2($lot_id, $wpis_historii);
    }
    return $result !== false;
}
