...
function srl_formatuj_status_lotu($status) {
    $statusy = array('wolny' => 'Dostępny do rezerwacji', 'zarezerwowany' => 'Zarezerwowany', 'zrealizowany' => 'Zrealizowany', 'przedawniony' => 'Przeterminowany');
    return isset($statusy[$status]) ? $statusy[$status] : ucfirst($status);
}
function srl_formatuj_status_slotu($status) {
    $statusy = array('Wolny' => 'Wolny', 'Prywatny' => 'Prywatny', 'Zarezerwowany' => 'Zarezerwowany', 'Zrealizowany' => 'Zrealizowany', 'Odwołany przez organizatora' => 'Odwołany przez organizatora');
...
function srl_formatuj_kategorie_wagowa($kategoria) {
    $kategorie = array('do_60kg' => 'do 60 kg', '60-80kg' => '60-80 kg', '80-100kg' => '80-100 kg', '100-120kg' => '100-120 kg', '120kg+' => '120 kg+');
    return isset($kategorie[$kategoria]) ? $kategorie[$kategoria] : $kategoria;
}
function srl_formatuj_sprawnosc_fizyczna($sprawnosc) {
    $sprawnosci = array('dobra' => 'Dobra', 'przecietna' => 'Przeciętna', 'ograniczona' => 'Ograniczona');
...
function srl_formatuj_date_i_czas_polski($data, $godzina_start) {
    ...
    $nazwy_miesiecy = ['stycznia', 'lutego', 'marca', 'kwietnia', 'maja', 'czerwca', 'lipca', 'sierpnia', 'września', 'października', 'listopada', 'grudnia'];
    ...
    return '<div class="srl-termin-data">' . $dzien . ' ' . $nazwy_miesiecy[$miesiac] . ' ' . $rok . ', godz: ' . $godzina . '</div>' . '<div class="srl-termin-dzien">' . $nazwy_dni[$dzien_tygodnia] . '</div>';
}
