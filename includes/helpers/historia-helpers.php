...
function srl_kategoryzuj_akcje($typ) {
    $kategorie = array('ZMIANA STATUSU' => array('rezerwacja_klient', 'anulowanie_klient', 'zmiana_statusu_admin', 'realizacja_admin', 'odwolanie_przez_organizatora', 'przywrocenie_przez_admin', 'anulowanie_przez_organizatora', 'przypisanie_admin'), 'DOKUPIENIE WARIANTU' => array('dokupienie_filmowanie', 'dokupienie_akrobacje', 'przedluzenie_waznosci', 'zmiana_opcji'), 'SYSTEMOWE' => array('przypisanie_id', 'opcja_przy_zakupie', 'zakup_lotu'), 'ZMIANA DANYCH' => array('edycja_danych_pasazera', 'aktualizacja_profilu', 'zapisanie_danych_klienta'));
...
function srl_formatuj_status_badge($status) {
    $statusy = array('wolny' => array('label' => 'WOLNY', 'color' => '#28a745'), 'zarezerwowany' => array('label' => 'ZAREZERWOWANY', 'color' => '#ffc107'), 'zrealizowany' => array('label' => 'ZREALIZOWANY', 'color' => '#007bff'), 'odwolany' => array('label' => 'ODWOŁANY', 'color' => '#dc3545'), 'przedawniony' => array('label' => 'PRZEDAWNIONY', 'color' => '#6c757d'));
    $config = isset($statusy[$status]) ? $statusy[$status] : array('label' => strtoupper($status), 'color' => '#6c757d');
    return sprintf('<span class="srl-status-badge" style="background: %s; color: white; padding: 2px 8px; border-radius: 12px; font-size: 11px; font-weight: bold;">%s</span>', $config['color'], $config['label']);
}
...
function srl_czy_duplikat($nowy_wpis, $istniejace_wpisy) {
    ...
    foreach ($istniejace_wpisy as $wpis) {
        ...
        if ($wpis['typ'] === $nowy_wpis['typ'] && $wpis['executor'] === $nowy_wpis['executor']) {
            if ($nowy_wpis['typ'] === 'zmiana_statusu_admin') {
                $nowe_szczegoly = $nowy_wpis['szczegoly'];
                $stare_szczegoly = $wpis['szczegoly'];
                if (isset($nowe_szczegoly['stary_status']) && isset($nowe_szczegoly['nowy_status']) && isset($stare_szczegoly['stary_status']) && isset($stare_szczegoly['nowy_status'])) {
                    if ($nowe_szczegoly['stary_status'] === $stare_szczegoly['stary_status'] && $nowe_szczegoly['nowy_status'] === $stare_szczegoly['nowy_status']) {
                        return true;
                    }
                }
            }
...
function srl_dopisz_do_historii_lotu_v2($lot_id, $wpis) {
    ...
    $aktualna_historia = $wpdb->get_var($wpdb->prepare("SELECT historia_modyfikacji FROM $tabela_loty WHERE id = %d", $lot_id));
    $historia = array();
    ...
    $result = $wpdb->update($tabela_loty, array('historia_modyfikacji' => json_encode($historia)), array('id' => $lot_id), array('%s'), array('%d'));
    if ($result !== false) {
    ...
    if (!empty($lot['historia_modyfikacji'])) {
        ...
        if (is_array($historia_json)) {
            foreach ($historia_json as $modyfikacja) {
                $events[] = array('timestamp' => strtotime($modyfikacja['data']), 'date' => $modyfikacja['data'], 'type' => $modyfikacja['typ'], 'action_name' => srl_formatuj_nazwe_akcji_v2($modyfikacja), 'executor' => isset($modyfikacja['executor']) ? $modyfikacja['executor'] : 'System', 'details' => srl_uproszczony_opis_akcji($modyfikacja['typ'], $modyfikacja['szczegoly']??array()), 'formatted_date' => date('d.m.Y H:i', strtotime($modyfikacja['data'])));
            }
        }
    }
...
function srl_wyczysc_historie_wszystkich_lotow() {
    ...
    global $wpdb;
    $tabela_loty = $wpdb->prefix . 'srl_zakupione_loty';
    $result = $wpdb->query("UPDATE $tabela_loty SET historia_modyfikacji = NULL");
    return $result;
}
function srl_przebuduj_historie_lotow() {
    ...
    foreach ($loty as $lot) {
        ...
        $historia_poczatkowa[] = array('data' => $lot['data_zakupu'], 'typ' => 'przypisanie_id', 'executor' => 'System', 'szczegoly' => array('lot_id' => $lot['id'], 'nazwa_produktu' => $lot['nazwa_produktu'], 'order_id' => $lot['order_id'], 'user_id' => $lot['user_id'], 'data_waznosci' => $lot['data_waznosci']));
        if ($lot['ma_filmowanie']) {
            $historia_poczatkowa[] = array('data' => $lot['data_zakupu'], 'typ' => 'opcja_przy_zakupie', 'executor' => 'System', 'szczegoly' => array('opcja' => 'filmowanie', 'dodano_przy_zakupie' => true, 'order_id' => $lot['order_id']));
        }
        if ($lot['ma_akrobacje']) {
            $historia_poczatkowa[] = array('data' => $lot['data_zakupu'], 'typ' => 'opcja_przy_zakupie', 'executor' => 'System', 'szczegoly' => array('opcja' => 'akrobacje', 'dodano_przy_zakupie' => true, 'order_id' => $lot['order_id']));
        }
        $wpdb->update($tabela_loty, array('historia_modyfikacji' => json_encode($historia_poczatkowa)), array('id' => $lot['id']), array('%s'), array('%d'));
        $przetworzone++;
    }
    return $przetworzone;
}
