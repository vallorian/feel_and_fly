...
function srl_wyslij_email_potwierdzenia($user_id, $slot, $lot) {
    ...
    $headers = array('Content-Type: text/plain; charset=UTF-8', 'From: ' . get_option('blogname') . ' <' . get_option('admin_email') . '>');
...
function srl_wyslij_email_administratora($temat, $wiadomosc, $dane_dodatkowe = array()) {
    ...
    $headers = array('Content-Type: text/plain; charset=UTF-8', 'From: System <noreply@' . wp_parse_url(home_url(), PHP_URL_HOST) . '>');
...
function srl_zastap_zmienne_w_emailu($tekst, $zmienne = array()) {
    $domyslne_zmienne = array('{SITE_NAME}' => get_option('blogname'), '{SITE_URL}' => home_url(), '{ADMIN_EMAIL}' => get_option('admin_email'), '{DATE}' => date('d.m.Y'), '{TIME}' => date('H:i'), '{DATETIME}' => date('d.m.Y H:i'));
...
function srl_sprawdz_czy_email_dozwolony($email) {
    ...
    $blacklisted_domains = array('tempmail.org', '10minutemail.com', 'guerrillamail.com', 'mailinator.com');
