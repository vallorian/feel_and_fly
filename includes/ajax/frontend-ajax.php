...
function srl_pobierz_dane_klienta() {
    ...
    $rezerwacje = $wpdb->get_results($wpdb->prepare("SELECT zl.*, t.data, t.godzina_start, t.godzina_koniec,
                v.kod_vouchera,
         ...
         ORDER BY t.data ASC, t.godzina_start ASC", $user_id), ARRAY_A);
    $dostepne_loty = $wpdb->get_results($wpdb->prepare("SELECT zl.*, v.kod_vouchera,
                v.buyer_imie as voucher_buyer_imie,
                v.buyer_nazwisko as voucher_buyer_nazwisko
         FROM $tabela_loty zl
         LEFT JOIN {$wpdb->prefix}srl_vouchery_upominkowe v ON zl.id = v.lot_id
    ...
    $dane_osobowe = array('imie' => get_user_meta($user_id, 'srl_imie', true), 'nazwisko' => get_user_meta($user_id, 'srl_nazwisko', true), 'rok_urodzenia' => get_user_meta($user_id, 'srl_rok_urodzenia', true), 'kategoria_wagowa' => get_user_meta($user_id, 'srl_kategoria_wagowa', true), 'sprawnosc_fizyczna' => get_user_meta($user_id, 'srl_sprawnosc_fizyczna', true), 'telefon' => get_user_meta($user_id, 'srl_telefon', true), 'uwagi' => get_user_meta($user_id, 'srl_uwagi', true));
    $dane_kompletne = !empty($dane_osobowe['imie']) && !empty($dane_osobowe['nazwisko']) && !empty($dane_osobowe['rok_urodzenia']) && !empty($dane_osobowe['kategoria_wagowa']) && !empty($dane_osobowe['sprawnosc_fizyczna']);
    wp_send_json_success(array('rezerwacje' => $rezerwacje, 'dostepne_loty' => $dostepne_loty, 'dane_osobowe' => $dane_osobowe, 'dane_kompletne' => $dane_kompletne));
}
function srl_zapisz_dane_pasazera() {
    ...
    $user_id = get_current_user_id();
    $dane = array('imie' => sanitize_text_field($_POST['imie']), 'nazwisko' => sanitize_text_field($_POST['nazwisko']), 'rok_urodzenia' => intval($_POST['rok_urodzenia']), 'kategoria_wagowa' => sanitize_text_field($_POST['kategoria_wagowa']), 'sprawnosc_fizyczna' => sanitize_text_field($_POST['sprawnosc_fizyczna']), 'telefon' => sanitize_text_field($_POST['telefon']), 'uwagi' => sanitize_textarea_field($_POST['uwagi']), 'akceptacja_regulaminu' => isset($_POST['akceptacja_regulaminu']) && $_POST['akceptacja_regulaminu'] === 'true');
    ...
    if ($czy_zmiana) {
        global $wpdb;
        $tabela_loty = $wpdb->prefix . 'srl_zakupione_loty';
        $loty_uzytkownika = $wpdb->get_results($wpdb->prepare("SELECT id FROM $tabela_loty WHERE user_id = %d AND status IN ('wolny', 'zarezerwowany') AND data_waznosci >= CURDATE()", $user_id), ARRAY_A);
        if (!empty($loty_uzytkownika)) {
            foreach ($loty_uzytkownika as $lot) {
                $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'zapisanie_danych_klienta', 'executor' => 'Klient', 'szczegoly' => array('zmienione_pola' => $zmienione_pola, 'user_id' => $user_id, 'stare_dane' => $stare_dane, 'nowe_dane' => $dane));
...
function srl_pobierz_dostepne_dni() {
    ...
    $wynik = $wpdb->get_results($wpdb->prepare("SELECT data, COUNT(*) as wolne_sloty
         FROM $tabela 
         ...
         ORDER BY data ASC", $poczatek_miesiaca, $koniec_miesiaca), ARRAY_A);
    $dostepne_dni = array();
...
function srl_pobierz_dostepne_godziny() {
    ...
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_terminy';
    $wolne_sloty = $wpdb->get_results($wpdb->prepare("SELECT id, pilot_id, godzina_start, godzina_koniec 
         FROM $tabela 
         WHERE data = %s 
         AND status = 'Wolny'
         ORDER BY pilot_id ASC, godzina_start ASC", $data), ARRAY_A);
...
function srl_zablokuj_slot_tymczasowo() {
    ...
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_terminy';
    $slot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela WHERE id = %d AND status = 'Wolny'", $termin_id), ARRAY_A);
    if (!$slot) {
    ...
    wp_send_json_success(array('slot' => $slot, 'blokada_do' => time() + 15 * MINUTE_IN_SECONDS));
}
function srl_dokonaj_rezerwacji() {
    ...
    $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_loty 
         WHERE id = %d AND user_id = %d AND status = 'wolny' AND data_waznosci >= CURDATE()", $lot_id, $user_id), ARRAY_A);
    if (!$lot) {
    ...
    try {
        ...
        $update_slot = $wpdb->update($tabela_terminy, array('status' => 'Zarezerwowany', 'klient_id' => $user_id), array('id' => $termin_id), array('%s', '%d'), array('%d'));
        if ($update_slot === false) {
            throw new Exception('Błąd aktualizacji slotu.');
        }
        $update_lot = $wpdb->update($tabela_loty, array('status' => 'zarezerwowany', 'data_rezerwacji' => $current_datetime, 'termin_id' => $termin_id, 'dane_pasazera' => json_encode($dane_pasazera)), array('id' => $lot_id), array('%s', '%s', '%d', '%s'), array('%d'));
        if ($update_lot === false) {
        ...
        $wpis_historii = array('data' => $current_datetime, 'typ' => 'rezerwacja_klient', 'executor' => 'Klient', 'szczegoly' => array('termin_id' => $termin_id, 'termin' => $termin_opis, 'pilot_id' => $slot['pilot_id'], 'data_lotu' => $slot['data'], 'godzina_start' => $slot['godzina_start'], 'godzina_koniec' => $slot['godzina_koniec'], 'user_id' => $user_id, 'dane_pasazera_zapisane' => !empty($dane_pasazera['imie']) && !empty($dane_pasazera['nazwisko']), 'stary_status' => 'wolny', 'nowy_status' => 'zarezerwowany'));
        ...
        wp_send_json_success(array('message' => 'Rezerwacja została potwierdzona!', 'slot' => $slot, 'lot' => $lot));
    }
    catch(Exception $e) {
        $wpdb->query('ROLLBACK');
...
function srl_anuluj_rezerwacje_klient() {
    ...
    $tabela_terminy = $wpdb->prefix . 'srl_terminy';
    $tabela_loty = $wpdb->prefix . 'srl_zakupione_loty';
    $lot = $wpdb->get_row($wpdb->prepare("SELECT zl.*, t.data, t.godzina_start, t.godzina_koniec, t.pilot_id
         FROM $tabela_loty zl
         LEFT JOIN $tabela_terminy t ON zl.termin_id = t.id
         WHERE zl.id = %d AND zl.user_id = %d AND zl.status = 'zarezerwowany'", $lot_id, $user_id), ARRAY_A);
    if (!$lot) {
