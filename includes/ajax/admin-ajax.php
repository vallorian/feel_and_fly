...
function srl_dodaj_godzine() {
    ...
    $result = $wpdb->insert($tabela, array('data' => $data, 'pilot_id' => $pilot_id, 'godzina_start' => $godzStart, 'godzina_koniec' => $godzKoniec, 'status' => $status, 'klient_id' => $klient_id), array('%s', '%d', '%s', '%s', '%s', '%d'));
    if ($result === false) {
...
function srl_zmien_slot() {
    ...
    $existing = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $tabela WHERE id = %d", $termin_id));
    if (!$existing) {
        wp_send_json_error('Slot nie istnieje.');
        return;
    }
    $dane = array('godzina_start' => $godzStart, 'godzina_koniec' => $godzKoniec, 'status' => $status, 'klient_id' => $klient_id ? $klient_id : null);
    $wynik = $wpdb->update($tabela, $dane, array('id' => $termin_id), array('%s', '%s', '%s', '%d'), array('%d'));
    if ($wynik === false) {
...
function srl_usun_godzine() {
    ...
    $tabela = $wpdb->prefix . 'srl_terminy';
    $termin_id = intval($_POST['termin_id']);
    $slot = $wpdb->get_row($wpdb->prepare("SELECT data, status, klient_id FROM $tabela WHERE id = %d", $termin_id), ARRAY_A);
    if (!$slot) {
        wp_send_json_error('Slot nie istnieje.');
        return;
    }
    ...
    $data = $slot['data'];
    $usun = $wpdb->delete($tabela, array('id' => $termin_id), array('%d'));
    if ($usun === false) {
...
function srl_zmien_status_godziny() {
    ...
    $slot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_terminy WHERE id = %d", $termin_id), ARRAY_A);
    if (!$slot) {
        wp_send_json_error('Slot nie istnieje.');
        return;
    }
    ...
    try {
        $dane_slotu = array('status' => $status, 'klient_id' => $klient_id ? $klient_id : null);
        if ($status === 'Wolny') {
            $dane_slotu['notatka'] = null;
        }
        $wynik = $wpdb->update($tabela_terminy, $dane_slotu, array('id' => $termin_id), array('%s', '%d'), array('%d'));
        if ($wynik === false) {
        ...
        if ($lot) {
            ...
            if (!empty($dane_lotu_update)) {
                $update_result = $wpdb->update($tabela_loty, $dane_lotu_update, array('id' => $lot['id']), array_fill(0, count($dane_lotu_update), '%s'), array('%d'));
                if ($update_result === false) {
                    throw new Exception('Błąd aktualizacji statusu lotu');
                }
            }
            if (!empty($nowy_status_lotu) && $stary_status !== $nowy_status_lotu) {
                $szczegoly = array('termin_id' => $termin_id, 'stary_status_slotu' => $slot['status'], 'nowy_status_slotu' => $status, 'stary_status' => $stary_status, 'nowy_status' => $nowy_status_lotu, 'zmiana_przez_admin' => true);
                $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'zmiana_statusu_admin', 'executor' => 'Admin', 'szczegoly' => $szczegoly);
...
function srl_anuluj_lot_przez_organizatora() {
    ...
    $lot = null;
    if ($klient_id > 0) {
        $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_loty WHERE termin_id = %d AND user_id = %d", $termin_id, $klient_id), ARRAY_A);
    }
    $wpdb->query('START TRANSACTION');
    try {
        ...
        $result = $wpdb->update($tabela_terminy, array('status' => 'Odwołany przez organizatora', 'notatka' => json_encode($dane_historyczne)), array('id' => $termin_id), array('%s', '%s'), array('%d'));
        if ($result === false) {
        ...
        if ($lot) {
            ...
            $wpdb->update($tabela_loty, array('status' => 'wolny', 'termin_id' => null, 'data_rezerwacji' => null), array('id' => $lot['id']), array('%s', '%d', '%s'), array('%d'));
            $user = get_userdata($klient_id);
            if ($user) {
                $to = $user->user_email;
                ...
                $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'odwolanie_przez_organizatora', 'executor' => 'Admin', 'szczegoly' => array('termin_id' => $termin_id, 'odwolany_termin' => $szczegoly_terminu, 'stary_status' => $stary_status, 'nowy_status' => 'wolny', 'klient_id' => $klient_id, 'email_wyslany' => true, 'slot_zachowany' => true));
                srl_dopisz_do_historii_lotu_v2($lot['id'], $wpis_historii);
            }
        }
        $wpdb->query('COMMIT');
...
function srl_ajax_zrealizuj_lot() {
    ...
    try {
        ...
        $wpdb->update($tabela_terminy, array('status' => 'Zrealizowany'), array('id' => $termin_id), array('%s'), array('%d'));
        $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_loty WHERE termin_id = %d", $termin_id), ARRAY_A);
        if ($lot) {
            $wpdb->update($tabela_loty, array('status' => 'zrealizowany'), array('id' => $lot['id']), array('%s'), array('%d'));
            $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'realizacja_admin', 'executor' => 'Admin', 'szczegoly' => array('termin_id' => $termin_id, 'stary_status' => 'zarezerwowany', 'nowy_status' => 'zrealizowany', 'lot_id' => $lot['id']));
            srl_dopisz_do_historii_lotu_v2($lot['id'], $wpis_historii);
        }
        $wpdb->query('COMMIT');
        $data = $slot['data'];
...
function srl_ajax_przypisz_wykupiony_lot() {
    ...
    $wpdb->query('START TRANSACTION');
    try {
        $slot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_terminy WHERE id = %d AND status = 'Wolny'", $termin_id), ARRAY_A);
        if (!$slot) {
            throw new Exception('Slot nie jest dostępny.');
        }
        $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_loty WHERE id = %d AND status = 'wolny'", $lot_id), ARRAY_A);
        if (!$lot) {
            throw new Exception('Lot nie jest dostępny.');
        }
        $wpdb->update($tabela_terminy, array('status' => 'Zarezerwowany', 'klient_id' => $lot['user_id']), array('id' => $termin_id), array('%s', '%d'), array('%d'));
        $wpdb->update($tabela_loty, array('status' => 'zarezerwowany', 'data_rezerwacji' => srl_get_current_datetime(), 'termin_id' => $termin_id), array('id' => $lot_id), array('%s', '%s', '%d'), array('%d'));
        $termin = $wpdb->get_row($wpdb->prepare("SELECT data, godzina_start, godzina_koniec, pilot_id FROM {$wpdb->prefix}srl_terminy WHERE id = %d", $termin_id), ARRAY_A);
        $termin_opis = sprintf('%s %s-%s (Pilot %d)', $termin['data'], substr($termin['godzina_start'], 0, 5), substr($termin['godzina_koniec'], 0, 5), $termin['pilot_id']);
        $wpis_historii = array('data' => srl_get_current_datetime(), 'typ' => 'przypisanie_admin', 'executor' => 'Admin', 'szczegoly' => array('termin_id' => $termin_id, 'termin' => $termin_opis, 'data_lotu' => $termin['data'], 'godzina_start' => $termin['godzina_start'], 'godzina_koniec' => $termin['godzina_koniec'], 'pilot_id' => $termin['pilot_id'], 'user_id' => $lot['user_id'], 'stary_status' => 'wolny', 'nowy_status' => 'zarezerwowany'));
        srl_dopisz_do_historii_lotu_v2($lot_id, $wpis_historii);
        $wpdb->query('COMMIT');
        $slot_date = $wpdb->get_var($wpdb->prepare("SELECT data FROM {$wpdb->prefix}srl_terminy WHERE id = %d", $termin_id));
        if ($slot_date) {
...
function srl_wyszukaj_klientow_loty() {
    ...
    global $wpdb;
    $tabela_loty = $wpdb->prefix . 'srl_zakupione_loty';
    $results = $wpdb->get_results($wpdb->prepare("SELECT DISTINCT u.ID, u.display_name, u.user_login 
         FROM {$wpdb->users} u
    ...
    foreach ($results as $user) {
        $wynik[] = array('id' => $user->ID, 'nazwa' => $user->display_name . ' (' . $user->user_login . ')');
    }
...
function srl_ajax_dodaj_voucher_recznie() {
    ...
    $exists = $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM $tabela WHERE kod_vouchera = %s", $kod_vouchera));
    if ($exists > 0) {
        wp_send_json_error('Voucher o tym kodzie już istnieje.');
    }
    $user_id = get_current_user_id();
    $user = get_userdata($user_id);
    $result = $wpdb->insert($tabela, array('order_item_id' => 0, 'order_id' => 0, 'buyer_user_id' => $user_id, 'buyer_imie' => $user->first_name ? : 'Admin', 'buyer_nazwisko' => $user->last_name ? : 'Manual', 'nazwa_produktu' => 'Voucher dodany ręcznie', 'kod_vouchera' => $kod_vouchera, 'status' => 'do_wykorzystania', 'data_zakupu' => srl_get_current_datetime(), 'data_waznosci' => $data_waznosci), array('%d', '%d', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s'));
    if ($result !== false) {
...
function srl_ajax_wyszukaj_dostepnych_klientow() {
    ...
    global $wpdb;
    $tabela_loty = $wpdb->prefix . 'srl_zakupione_loty';
    $results = $wpdb->get_results($wpdb->prepare("SELECT zl.id as lot_id, zl.nazwa_produktu, zl.user_id,
         ...
         ORDER BY u.display_name
         LIMIT 20", '%' . $query . '%', '%' . $query . '%', '%' . $query . '%', '%' . $query . '%'), ARRAY_A);
    $wynik = array();
    foreach ($results as $row) {
        $telefon = get_user_meta($row['user_id'], 'srl_telefon', true);
        $wynik[] = array('lot_id' => $row['lot_id'], 'user_id' => $row['user_id'], 'nazwa' => $row['display_name'] . ' (' . $row['user_email'] . ')' . ($telefon ? ' - ' . $telefon : ''), 'produkt' => $row['nazwa_produktu']);
    }
    wp_send_json_success($wynik);
}
function srl_ajax_przypisz_klienta_do_slotu() {
    ...
    $wpdb->query('START TRANSACTION');
    try {
        $update_slot = $wpdb->update($tabela_terminy, array('status' => 'Zarezerwowany', 'klient_id' => $lot['user_id']), array('id' => $termin_id), array('%s', '%d'), array('%d'));
        if ($update_slot === false) {
            throw new Exception('Błąd aktualizacji slotu.');
        }
        $update_lot = $wpdb->update($tabela_loty, array('status' => 'zarezerwowany', 'data_rezerwacji' => srl_get_current_datetime(), 'termin_id' => $termin_id), array('id' => $lot_id), array('%s', '%s', '%d'), array('%d'));
        if ($update_lot === false) {
...
function srl_ajax_zapisz_dane_prywatne() {
    ...
    $dane_pasazera = array('imie' => $imie, 'nazwisko' => $nazwisko, 'rok_urodzenia' => $rok_urodzenia, 'telefon' => $telefon, 'sprawnosc_fizyczna' => $sprawnosc_fizyczna, 'kategoria_wagowa' => $kategoria_wagowa, 'uwagi' => $uwagi, 'typ' => 'prywatny');
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_terminy';
    $result = $wpdb->update($tabela, array('status' => 'Prywatny', 'notatka' => json_encode($dane_pasazera)), array('id' => $termin_id), array('%s', '%s'), array('%d'));
    if ($result !== false) {
...
function srl_ajax_pobierz_dane_prywatne() {
    ...
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_terminy';
    $notatka = $wpdb->get_var($wpdb->prepare("SELECT notatka FROM $tabela WHERE id = %d", $termin_id));
    if ($notatka) {
...
function srl_ajax_wyszukaj_wolne_loty() {
    ...
    $results = $wpdb->get_results($wpdb->prepare("SELECT zl.id as lot_id, zl.order_id, zl.user_id, zl.imie, zl.nazwisko, 
...
function srl_ajax_przywroc_rezerwacje() {
    ...
    $tabela_terminy = $wpdb->prefix . 'srl_terminy';
    $tabela_loty = $wpdb->prefix . 'srl_zakupione_loty';
    $slot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_terminy WHERE id = %d AND status = 'Odwołany przez organizatora'", $termin_id), ARRAY_A);
    if (!$slot) {
    ...
    $lot = $wpdb->get_row($wpdb->prepare("SELECT * FROM $tabela_loty WHERE id = %d AND user_id = %d", $lot_id, $klient_id), ARRAY_A);
    if (!$lot) {
    ...
    $wpdb->query('START TRANSACTION');
    try {
        $result_slot = $wpdb->update($tabela_terminy, array('status' => 'Zarezerwowany', 'notatka' => null), array('id' => $termin_id), array('%s', '%s'), array('%d'));
        if ($result_slot === false) {
            throw new Exception('Błąd przywracania slotu.');
        }
        $result_lot = $wpdb->update($tabela_loty, array('status' => 'zarezerwowany', 'termin_id' => $termin_id, 'data_rezerwacji' => srl_get_current_datetime()), array('id' => $lot_id), array('%s', '%d', '%s'), array('%d'));
        if ($result_lot === false) {
        ...
        if ($user) {
            ...
            wp_mail($to, $subject, $body);
            $wpis_historii = array('data' => srl_get_current_datetime(), 'opis' => "Rezerwacja przywrócona przez administratora - termin {$szczegoly_terminu}", 'typ' => 'przywrocenie_przez_admin', 'executor' => 'Admin', 'szczegoly' => array('termin_id' => $termin_id, 'przywrocony_termin' => $szczegoly_terminu, 'klient_id' => $klient_id, 'email_wyslany' => true, 'powod' => 'Przywrócenie po odwołaniu przez organizatora'));
            srl_dopisz_do_historii_lotu_v2($lot_id, $wpis_historii);
        }
        $wpdb->query('COMMIT');
        srl_zwroc_godziny_wg_pilota($slot['data']);
...
function srl_ajax_pobierz_dane_odwolanego() {
    ...
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_terminy';
    $notatka = $wpdb->get_var($wpdb->prepare("SELECT notatka FROM $tabela WHERE id = %d AND status = 'Odwołany przez organizatora'", $termin_id));
    if ($notatka) {
        $dane = json_decode($notatka, true);
        if ($dane && is_array($dane)) {
            wp_send_json_success($dane);
...
function srl_ajax_zrealizuj_lot_prywatny() {
    ...
    try {
        ...
        $result = $wpdb->update($tabela_terminy, array('status' => 'Zrealizowany'), array('id' => $termin_id), array('%s'), array('%d'));
        if ($result === false) {
            throw new Exception('Błąd aktualizacji statusu slotu.');
        }
        $data = $slot['data'];
