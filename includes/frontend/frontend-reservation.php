...
function srl_pobierz_dane_klienta() {
	...
	// 1. Pobierz aktualne rezerwacje z dodatkowymi danymi o opcjach
	...
	// 2. Pobierz dostępne loty do rezerwacji z dodatkowymi danymi o opcjach
    ...
    if (WC()->cart) {
        foreach (WC()->cart->get_cart() as $cart_item_key => $cart_item) {
            if (isset($cart_item['srl_lot_id'])) {
                ...
                // ID produktów opcji
                if ($product_id == 116) { // Filmowanie
                    $opcje_w_koszyku[$lot_id]['filmowanie'] = true;
                } elseif ($product_id == 117) { // Akrobacje
                    $opcje_w_koszyku[$lot_id]['akrobacje'] = true;
                } elseif ($product_id == 115) { // Przedłużenie
...
// AJAX: Sprawdź i dodaj opcję do koszyka (tylko jeśli jeszcze nie ma)
