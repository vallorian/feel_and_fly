<?php add_action('admin_menu', 'srl_dodaj_strony_admin');
function srl_dodaj_strony_admin() {
    add_menu_page('Rezerwacje lotów tandemowych', 'Rezerwacje Tandem', 'manage_options', 'srl-dashboard', 'srl_wyswietl_dashboard', 'dashicons-calendar', 56);
    add_submenu_page('srl-dashboard', 'Kalendarz', 'Kalendarz', 'manage_options', 'srl-kalendarz', 'srl_wyswietl_kalendarz');
    add_submenu_page('srl-dashboard', 'Planowanie dni', 'Dzień tygodnia', 'manage_options', 'srl-dzien', 'srl_wyswietl_dzien');
    add_submenu_page('srl-dashboard', 'Wykupione loty', 'Wykupione loty', 'manage_options', 'srl-wykupione-loty', 'srl_wyswietl_wykupione_loty');
    add_submenu_page('srl-dashboard', 'Synchronizacja', 'Synchronizacja', 'manage_options', 'srl-sync-flights', 'srl_wyswietl_synchronizacje');
    add_submenu_page('srl-dashboard', 'Vouchery oczekujące', 'Oczekujące na potwierdzenie', 'manage_options', 'srl-voucher', 'srl_wyswietl_vouchery');
    if (function_exists('srl_voucher_table_exists') && srl_voucher_table_exists()) {
        add_submenu_page('srl-dashboard', 'Zakupione Vouchery', 'Zakupione Vouchery', 'manage_options', 'srl-zakupione-vouchery', 'srl_wyswietl_zakupione_vouchery');
    }
}
...
function srl_enqueue_admin_scripts($hook) {
    ...
    if (isset($_GET['page']) && $_GET['page'] === 'srl-dzien') {
        ...
        $istniejace_godziny = $wpdb->get_results($wpdb->prepare("SELECT t.id, t.pilot_id, t.godzina_start, t.godzina_koniec, t.status, t.klient_id, t.notatka,
          ...
          WHERE t.data = %s
          ORDER BY t.pilot_id ASC, t.godzina_start ASC", $data), ARRAY_A);
        ...
        foreach ($istniejace_godziny as $wiersz) {
            ...
            if (($wiersz['status'] === 'Zarezerwowany' || $wiersz['status'] === 'Zrealizowany') && intval($wiersz['klient_id']) > 0) {
                ...
                if ($user) {
                    ...
                    $lot = $wpdb->get_var($wpdb->prepare("SELECT id FROM {$wpdb->prefix}srl_zakupione_loty WHERE termin_id = %d AND status IN ('zarezerwowany', 'zrealizowany')", $wiersz['id']));
                    ...
                    if (empty($dane_pasazera_cache['imie'])) {
                        $dane_pasazera_cache = array('imie' => $imie, 'nazwisko' => $nazwisko, 'rok_urodzenia' => get_user_meta(intval($wiersz['klient_id']), 'srl_rok_urodzenia', true), 'telefon' => get_user_meta(intval($wiersz['klient_id']), 'srl_telefon', true), 'kategoria_wagowa' => get_user_meta(intval($wiersz['klient_id']), 'srl_kategoria_wagowa', true), 'sprawnosc_fizyczna' => get_user_meta(intval($wiersz['klient_id']), 'srl_sprawnosc_fizyczna', true), 'uwagi' => get_user_meta(intval($wiersz['klient_id']), 'srl_uwagi', true));
                    }
                }
            ...
            $godziny_wg_pilota[$pid][] = array('id' => intval($wiersz['id']), 'start' => substr($wiersz['godzina_start'], 0, 5), 'koniec' => substr($wiersz['godzina_koniec'], 0, 5), 'status' => $wiersz['status'], 'klient_id' => intval($wiersz['klient_id']), 'klient_nazwa' => $klient_nazwa, 'link_zamowienia' => $link_zamowienia, 'lot_id' => $lot_id, 'notatka' => $wiersz['notatka'], 'dane_pasazera_cache' => $dane_pasazera_cache);
        }
        $domyslna_liczba = 1;
        ...
        wp_localize_script('srl-admin-day', 'srlAdmin', array('ajaxurl' => admin_url('admin-ajax.php'), 'adminUrl' => admin_url(), 'nonce' => wp_create_nonce('srl_admin_nonce'), 'data' => $data, 'istniejaceGodziny' => $godziny_wg_pilota, 'domyslnaLiczbaPilotow' => $domyslna_liczba));
        wp_enqueue_style('srl-admin-style', SRL_PLUGIN_URL . 'assets/css/style.css');
    }
    ...
    if (!isset($_GET['page']) || $_GET['page'] !== 'srl-dzien') {
        wp_localize_script('srl-admin-day', 'srlAdmin', array('ajaxurl' => admin_url('admin-ajax.php'), 'adminUrl' => admin_url(), 'nonce' => wp_create_nonce('srl_admin_nonce'), 'data' => date('Y-m-d'), 'istniejaceGodziny' => array(), 'domyslnaLiczbaPilotow' => 1));
    }
}
...
function srl_enqueue_frontend_scripts() {
    ...
    if ($should_load) {
        ...
        wp_localize_script('srl-frontend-calendar', 'srlFrontend', array('ajaxurl' => admin_url('admin-ajax.php'), 'nonce' => wp_create_nonce('srl_frontend_nonce'), 'user_id' => get_current_user_id(), 'productIds' => srl_get_flight_option_product_ids()));
