...
function srl_dodaj_strony_admin() {
    add_menu_page(
        ...
        'srl-kalendarz', 'srl_wyswietl_kalendarz', 'dashicons-calendar', 56
    );

    $submenu_pages = [
        ['Planowanie dni', 'srl-dzien', 'srl_wyswietl_dzien'],
        ['Wykupione loty', 'srl-wykupione-loty', 'srl_wyswietl_wykupione_loty'],
        ['Vouchery', 'srl-voucher', 'srl_wyswietl_vouchery']
    ];
...
function srl_prepare_day_data($handle) {
    $data = (isset($_GET['data']) && preg_match('/^\d{4}-\d{2}-\d{2}$/', $_GET['data'])) 
            ? sanitize_text_field($_GET['data']) 
            : date('Y-m-d');

    $godziny_wg_pilota = srl_get_day_schedule_optimized($data);
    $domyslna_liczba = srl_calculate_default_pilots($godziny_wg_pilota);
...
function srl_calculate_default_pilots($godziny_wg_pilota) {
