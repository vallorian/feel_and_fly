<?php function srl_wyswietl_kalendarz() {
    ...
    $rezultaty = $wpdb->get_results($wpdb->prepare("SELECT data,
          ...
          GROUP BY data", $poczatek_miesiaca, $koniec_miesiaca), ARRAY_A);
    $dane_miesiac = array();
    foreach ($rezultaty as $wiersz) {
        $dane_miesiac[$wiersz['data']] = array('wszystkie' => intval($wiersz['ilosc_wszystkich']), 'wolne' => intval($wiersz['ilosc_wolnych']), 'zarezerwowane' => intval($wiersz['ilosc_zarezerwowanych']), 'prywatne' => intval($wiersz['ilosc_prywatnych']), 'zrealizowane' => intval($wiersz['ilosc_zrealizowanych']), 'odwolane' => intval($wiersz['ilosc_odwolanych']), 'zarezerwowane_razem' => intval($wiersz['ilosc_zarezerwowanych']) + intval($wiersz['ilosc_prywatnych']));
    }
    ...
    echo '<a class="button" href="' . esc_url(add_query_arg(array('miesiac' => date('n', $poprzedni_miesiac), 'rok' => date('Y', $poprzedni_miesiac)))) . '">&laquo; Poprzedni miesiąc</a> ';
    echo '<span style="margin:0 10px;"><strong>' . date('F Y', $pierwszy_dzien_miesiaca) . '</strong></span>';
    echo '<a class="button" href="' . esc_url(add_query_arg(array('miesiac' => date('n', $nastepny_miesiac), 'rok' => date('Y', $nastepny_miesiac)))) . '">Następny miesiąc &raquo;</a>';
    echo '</div>';
    ...
    for ($wiersz = 0;$wiersz < $liczba_wierszy;$wiersz++) {
        ...
        for ($kol = 1;$kol <= 7;$kol++) {
            ...
            if ($komorka_index <= $wypełnienie_puste || $dzień_licznik > $dni_w_miesiacu) {
                ...
                $dane_dnia = isset($dane_miesiac[$data_porownaj]) ? $dane_miesiac[$data_porownaj] : array('wszystkie' => 0, 'wolne' => 0, 'zarezerwowane' => 0, 'prywatne' => 0, 'zrealizowane' => 0, 'odwolane' => 0, 'zarezerwowane_razem' => 0);
