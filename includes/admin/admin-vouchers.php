...
function srl_render_voucher_status_badge($status, $type) {
    if ($type === 'partner') {
        $config = [
...
function srl_render_voucher_tabs($active_tab) {
    $tabs = [
...
function srl_wyswietl_vouchery_partnera() {
    ...
    echo srl_generate_select('status_filter', [
        '' => 'Wszystkie statusy',
        'oczekuje' => 'Oczekuje',
    ...
    if (empty($vouchery_partnera)) {
        ...
        foreach ($vouchery_partnera as $voucher) {
            ...
            if ($voucher['status'] === 'oczekuje') {
                ...
                echo esc_html($voucher['data_waznosci_vouchera'] ? srl_formatuj_date($voucher['data_waznosci_vouchera']) : 'Brak');
            }
            ...
            echo '<td>' . esc_html(srl_formatuj_date($voucher['data_zgloszenia'], 'd.m.Y H:i')) . '</td>';
            echo '<td>' . srl_render_voucher_status_badge($voucher['status'], 'partner') . '</td>';
            echo '<td>' . srl_generate_partner_voucher_actions($voucher) . '</td>';
            echo '</tr>';
        }
    }
...
function srl_wyswietl_zakupione_vouchery() {
    ...
    srl_handle_voucher_bulk_actions();
    
    $pagination_data = srl_get_voucher_pagination_data();
    $vouchery = srl_get_vouchers_with_filters($pagination_data);
    $stats = srl_get_voucher_stats();
    
    srl_render_voucher_stats($stats);
    srl_render_voucher_filters($pagination_data);
    srl_render_voucher_table($vouchery, $pagination_data);
    srl_render_voucher_modals();
    srl_render_partner_voucher_js(); // Dodajemy JavaScript na końcu
}

function srl_generate_partner_voucher_actions($voucher) {
    $actions = [];
...
function srl_handle_voucher_bulk_actions() {
...
function srl_get_voucher_pagination_data() {
    return [
        'per_page' => 20,
...
function srl_get_vouchers_with_filters($pagination) {
    global $wpdb;
    $tabela = $wpdb->prefix . 'srl_vouchery_upominkowe';
...
function srl_get_voucher_stats() {
    global $wpdb;
    return srl_execute_query(
...
function srl_render_voucher_stats($stats) {
    $status_labels = [
...
function srl_render_voucher_filters($pagination) {
    echo '<div class="tablenav top">';
    ...
    echo srl_generate_button('Filtruj', 'button', ['type' => 'submit']);
    
    if ($pagination['status_filter'] || $pagination['search']) {
        echo srl_generate_link(admin_url('admin.php?page=srl-voucher&tab=upominkowe'), 'Wyczyść filtry', 'button');
    }
    
    echo '</form></div>';
}

function srl_render_voucher_table($vouchery, $pagination) {
    ...
    if (empty($vouchery)) {
        ...
        if ($pagination['search'] || $pagination['status_filter']) {
            echo '<p>' . srl_generate_link(admin_url('admin.php?page=srl-voucher&tab=upominkowe'), 'Wyczyść filtry') . '</p>';
        }
        echo '</td></tr>';
    } else {
        foreach ($vouchery as $voucher) {
            srl_render_voucher_row($voucher);
        }
    }
...
function srl_render_voucher_row($voucher) {
	...
	if ($voucher['order_id'] == 0) {
		...
		echo '<small>Zam: ' . srl_generate_link($order_url, '#' . $voucher['order_id'], '', ['target' => '_blank']) . '</small>';
	}
	echo '</td>';
    echo '<td><strong>' . srl_generate_link(
	...
	if ($voucher['ma_filmowanie'] || $voucher['ma_akrobacje']) {
		echo '<br><small style="color: #0073aa;">' . srl_format_flight_options_html($voucher['ma_filmowanie'], $voucher['ma_akrobacje']) . '</small>';
	}
	echo '</td>';
    ...
    echo '<td>' . srl_render_voucher_status_badge($voucher['status'], 'upominkowe') . '</td>';
    echo '<td>' . srl_formatuj_date($voucher['data_zakupu'], 'd.m.Y H:i') . '</td>';
    echo '<td>' . srl_formatuj_waznosc_lotu($voucher['data_waznosci']) . '</td>';
    
    echo '<td>';
    if ($voucher['status'] === 'wykorzystany' && $voucher['user_display_name']) {
        echo '<strong>' . srl_generate_link(
        ...
        if ($voucher['data_wykorzystania']) {
            echo '<br><small style="color: #666;">Wykorzystano: ' . srl_formatuj_date($voucher['data_wykorzystania'], 'd.m.Y H:i') . '</small>';
        }
    } else {
...
function srl_render_voucher_modals() {
...
function srl_render_partner_voucher_modals() {
    ...
    <div id="passenger-details-modal" style="display: none;">
        <div class="wp-dialog" style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: white; padding: 20px; border-radius: 8px; box-shadow: 0 4px 20px rgba(0,0,0,0.3); z-index: 100000; max-width: 600px; max-height: 80vh; overflow-y: auto;">
            ...
            <div id="passenger-details-content">Ładowanie...</div>
        </div>
...
function srl_render_partner_voucher_js() {
    ...
    function showPassengerDetails(voucherId) {
        ...
        // Debug - sprawdź czy JS się ładuje
        console.log('SRL Vouchers JS loaded');
        
        // Sprawdź czy przycisk istnieje
        console.log('Przycisk voucher:', $('#srl-dodaj-voucher-recznie').length);
                html += '</div>';
    ...
    jQuery(document).ready(function($) {
        // Select all checkbox dla voucherów upominkowych
        $('#cb-select-all-1').on('change', function() {
            $('input[name="voucher_ids[]"]').prop('checked', $(this).is(':checked'));
        });
        
        // MODAL VOUCHERA - obsługa przycisku
        ...
        // MODAL VOUCHERA - obsługa zamykania
        $(document).on('click', '#srl-anuluj-voucher', function(e) {
            e.preventDefault();
            console.log('Anuluj kliknięty'); // Debug
            $('#srl-modal-voucher').hide();
            $('#srl-form-voucher-recznie')[0].reset();
        });
        
        // MODAL VOUCHERA - zamknij po kliknięciu tła
        ...
        // MODAL VOUCHERA - obsługa formularza
        $(document).on('submit', '#srl-form-voucher-recznie', function(e) {
            ...
            // Dodaj informacje o opcjach do nazwy produktu
            var opcjeText = '';
            ...
            $.post(ajaxurl, {
				...
				buyer_imie: '<?php echo wp_get_current_user()->first_name ?: "Admin"; ?>',
				buyer_nazwisko: '<?php echo wp_get_current_user()->last_name ?: "Manual"; ?>',
...
// Obsługa zatwierdzania/odrzucania voucherów
...
// Fallback function dla starych menu
function srl_wyswietl_zakupione_vouchery_old() {
    $tab = 'upominkowe';
    echo '<script>window.location.href = "' . admin_url('admin.php?page=srl-voucher&tab=upominkowe') . '";</script>';
}