...
class SRL_Admin_Flights {
    ...
    public static function wyswietlWykupioneLoty() {
		...
		jQuery(document).ready(function($) {
			...
			$(document).on("click", ".srl-add-option", function(e) {
				...
				if (!confirm("Czy na pewno dodać opcję " + opcja + " do tego lotu?")) {
					return;
				}
			...
			$(document).on("click", ".srl-remove-option", function(e) {
				...
				if (!confirm("Czy na pewno usunąć opcję " + opcja + " z tego lotu?")) {
					return;
				}
				...
				$.ajax({
					...
					success: function(response) {
						if (response.success) {
							alert("Opcja została usunięta!");
							location.reload();
						} else {
							alert("Błąd: " + response.data);
							button.prop("disabled", false).text("Usuń " + opcja);
						}
					},
					error: function() {
						alert("Błąd połączenia z serwerem");
    ...
    private function renderFilters($pagination) {
        ...
        echo '<input type="search" name="s" value="' . esc_attr($pagination['search']) . '" placeholder="Wprowadź szukaną frazę...">';
    ...
    private function renderDateRangePanel($pagination) {
        ...
        echo '</div>';
        echo '<div>';
        echo SRL_Helpers::getInstance()->generateButton('Wyczyść', 'button', ['type' => 'button', 'id' => 'srl-clear-dates', 'style' => 'margin-right: 10px;']);
        ...
        jQuery(document).ready(function($) {
            ...
            function pokazHistorieLotu(historia) {
                ...
                if (historia.events.length === 0) {
                    ...
                    content += "<thead><tr style=\"background: #f8f9fa;\"><th style=\"padding: 12px 8px; text-align: left; border-bottom: 1px solid #e1e5e9;\">Data</th><th style=\"padding: 12px 8px; text-align: left; border-bottom: 1px solid #e1e5e9;\">Akcja</th><th style=\"padding: 12px 8px; text-align: left; border-bottom: 1px solid #e1e5e9;\">Wykonawca</th><th style=\"padding: 12px 8px; text-align: left; border-bottom: 1px solid #e1e5e9;\">Szczegóły</th></tr></thead>";
                    content += "<tbody>";
